...
Next.js app (next-app/)         ──►  PostgreSQL (Docker: postgres-client, via Prisma)
   │  app/api/v1/**/route.ts    ──►  Gemini API (AI pitches)
...
next-app/
├── package.json
├── .env                        # DATABASE_URL, GEMINI_API_KEY, GOOGLE_PLACES_API_KEY, SIDECAR_URL — never committed
...
└── src/
    ...
    │   └── services/               # external I/O: scraper.ts (Places), pitch.ts (Gemini), whatsapp.ts (sidecar client), send-queue.ts (rate-limited worker — uses the `*Internal` unscoped repo fns, see .ai/patterns.md §12)
...
- **`lib/services/*.ts`**: external I/O (Gemini, Google Places, the Baileys sidecar) and the long-running send-queue worker. No Next.js request/response types in here.
