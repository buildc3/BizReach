...
- **2026-07-18 update:** strategy.md §8 (multi-tenant data isolation) and §9 (JWT auth) are now **done**, landed together in one pass rather than as separately sequenced phases — see strategy.md §8/§9 for the full detail and the "as actually executed" correction notes. The app now requires signup/login; every resource is scoped to the logged-in user.
...
| 8 | Auth + multi-tenant data isolation (strategy.md §8 + §9, done together) | ✅ | `User` model, JWT cookie auth, every `lib/repo/*.ts` fn scoped by `userId`, all 27 `/api/v1/*` handlers gated via `requireUser`, `src/proxy.ts`, `/login` + `/signup` pages, Sidebar logout. WhatsApp sidecar stays single-shared-instance (§8.5 Option A by default, not yet B/C). Verified via HTTP smoke test; user's own browser click-through still pending. |
...
- **2026-07-18** — Resumed from an in-flight, uncommitted state: an earlier session had already applied a live DB migration (`20260717161119_add_users_and_tenancy`) combining the `User` model (§9) with `userId` columns on `Search`/`Product`/`Lead`/`Template`/`SenderProfile` (§8) — going further than either section's plan called for on its own, and leaving the repo layer un-updated (the build was broken: `tsc` failed on `searches.ts`/`leads.ts`/`products.ts`/`templates.ts` missing the now-required `userId` field). Given the schema was already live, finished the combined scope rather than rolling back:
  - Every `lib/repo/*.ts` function now takes `userId` and scopes its query — direct `where: { id, userId }` for `Search`/`Product`/`Template`/`Lead`, `upsert`-by-`userId` for `SenderProfile` (was a singleton; now lazily creates a row per user), parent-join scoping for `Group` (`search.userId`) and `Message` (`lead.userId`), since neither has its own `userId` column. Mutations use `updateMany`/`deleteMany` + count-check so a cross-tenant ID 404s instead of raw-erroring.
  - Added `getMessageInternal`/`updateMessageStatusInternal`/`getLeadInternal` unscoped variants for `lib/services/send-queue.ts`'s worker, which has no request context and only ever acts on IDs already ownership-checked at enqueue time.
  - All 27 `/api/v1/*` route handlers gated with `const userId = await requireUser(req)`, threaded into the repo calls; `products/route.ts` (POST) and `templates/route.ts`/`[id]` now also verify a referenced `productId` belongs to the caller before linking it.
  - `src/proxy.ts` added (Next 16's renamed middleware) — redirects unauthenticated page requests to `/login`, 401s unauthenticated `/api/v1/*` requests; matcher exempts `/login`, `/signup`, `/api/auth/*`.
  - Frontend: `lib/api.ts` gained an `auth` client + a 401→`/login` redirect in `request<T>`; `hooks/useAuth.ts` added; existing pages moved into an `app/(app)/` route group whose `layout.tsx` now carries the Sidebar (root `layout.tsx` trimmed to just `Providers`); new `app/(auth)/login` and `/signup` pages (RHF + zod, no Sidebar); Sidebar gained a logout button + current-user email.
  - Verified `tsc --noEmit` and `next build` clean, then ran an HTTP smoke test against a temporary dev server (port 3001, since 3000 was already in use by another running instance): unauthenticated `/api/v1/*` → 401, unauthenticated page → 307 to `/login`, signup → cookie set → `/me` works, created data scoped to that user, a second user's list is empty and can't read the first user's search by ID (404), a fresh signup's sender-profile GET lazily creates a row, duplicate-email signup → 409, wrong password → generic 401. Test rows cleaned up from the DB afterward; smoke-test dev server process stopped.
  - Updated `.ai/architecture.md` (new folder shapes: `(app)`/`(auth)` route groups, `proxy.ts`, `auth.ts`), `.ai/patterns.md` (new §12: auth gate + per-user scoping pattern), `.ai/review-rules.md` (tenancy-scoping checklist items), `.ai/glossary.md` (new `User` entity, ownership notes on every model), and `strategy.md` §8/§9 status + "as actually executed" correction notes.
